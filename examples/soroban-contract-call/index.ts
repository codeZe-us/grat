...
async function run() {
  ...
  const contractId = 'CDVNDP6Z2F6W7K4W7C67VGLYI6W7S5Z6X6S7C66W7C67VGLYI6W7S5Z6'; // Example ID
  ...
  const userInfo = await (await fetch(`https://horizon-testnet.stellar.org/accounts/${user.publicKey()}`)).json();
  
  const tx = new TransactionBuilder(
    new Account(user.publicKey(), userInfo.sequence),
