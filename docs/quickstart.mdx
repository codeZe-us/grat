...
<Steps>
  <Step title="Start the Relay">
    Clone the Grat repository and start the relay server using Docker Compose.
    ...
    This starts the relay, a PostgreSQL database, and a Redis instance. The relay will automatically fund its internal channel accounts using the Stellar Friendbot.
  </Step>
...
<Info>
Managed cloud hosting is currently under development. For now, self-hosting via Docker is the best way to get started with Grat.
</Info>
